...
# Model file resolution: .keras -> TensorFlow, .pkl -> pickled scikit-learn
# estimator (e.g. the PCA + Random Forest pipeline). Default thresholds are the
# models' calibrated operating points (FPR <= 0.3% on the test benign set).
DEFAULT_THRESHOLDS = {'.keras': 0.59, '.pkl': 0.52}
...
def default_threshold(model_path):
    return DEFAULT_THRESHOLDS.get(os.path.splitext(model_path)[1], 0.59)

def predict_probs(model, scaled_flows):
    if hasattr(model, 'predict_proba'): # scikit-learn
        return model.predict_proba(scaled_flows)[:, 1]
    return model(scaled_flows, training=False).numpy().ravel()

# Function to classify a single flow
def classify_single_flow(flow, model, scaler, threshold=0.59):
    # Scale the flow features
    scaled_flow = scaler.transform([flow])
    prediction_prob = predict_probs(model, scaled_flow)[0]
...
def classify_flows(flows, model, scaler, threshold=0.59):
    # Scale the flow features
    scaled_flows = scaler.transform(flows)
    prediction_probs = predict_probs(model, scaled_flows)
...
def analyze_flows(network_flows, event_log_file, output_filter="all",
                  model_path=None, scaler_path=None, batch_size=64, threshold=None):
    try:
        _analyze_flows(network_flows, event_log_file, output_filter,
                       model_path, scaler_path, batch_size, threshold)
...
def _analyze_flows(network_flows, event_log_file, output_filter,
    ...
    if threshold is None:
        threshold = default_threshold(model_path)
    log.info(f"Classification model: {model_path}, threshold: {threshold}")
